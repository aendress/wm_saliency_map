function [proportion_greater_than_reference] = knops_number_comparison(sequentialPresentation, letNetworkSettleDuringSequentialPresentation, b_arg, useDist, snoise_arg,  activation_threshold_arg, total_time_steps_arg, max_inhibition_distance_arg)
...
%                Rows: set-size
    ...
    function [compared_array] = compare_to_reference(comparandum, reference, difference_threshold)
        % [compared_array] = compare_to_reference(comparandum, reference, difference_threshold)
        % Return array of 0's, .5's and 1's depending on whether the values of the
        % comparandom are below, at or above the reference by the difference
        % threshold
        ...
        compared_array = .5 * ones(size(comparandum));
        
        compared_array = compared_array - (((comparandum + difference_threshold) < reference) * 0.5) ...
                                        + (((comparandum - difference_threshold) > reference) * 0.5);
...
nsim = 100;
difference_threshold = .0001;

proportion_greater_than_reference = zeros(MAX_SET_SIZE, 2);

[ma_reference] = run_simulation(reference_numerosity, dist, useDist, sequentialPresentation, letNetworkSettleDuringSequentialPresentation, nsim);   
ma_reference_mean = mean(ma_reference(:,1));
...
for ss=1:MAX_SET_SIZE
    ...
    ma_tmp = zeros(nsim,2);
    for main_sim=1:nsim
        [ma_tmp(main_sim,:)] = run_simulation(ss, dist, useDist, sequentialPresentation, letNetworkSettleDuringSequentialPresentation, 1);
    end;    
    
    proportion_greater_than_than_reference_array = compare_to_reference(ma_tmp(:,1), ma_reference_mean, difference_threshold);
    proportion_greater_than_reference(ss,:) = [mean(proportion_greater_than_than_reference_array) std(proportion_greater_than_than_reference_array)/sqrt(nsim)];
